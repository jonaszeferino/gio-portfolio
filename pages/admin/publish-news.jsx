...
const App = () => {
  ...
  const [siteArticle, setSiteArticle] = useState({}); // Estado para armazenar o valor do site_article
    ...
    try {
      ...
      if (response.ok) {
        ...
        dataRecive.forEach((news) => {
          ...
          initialSiteArticle[news.id] = news.site_article;
        });
        setDateTime(initialDateTimes);
        setVisibility(initialVisibility);
        setSiteArticle(initialSiteArticle);
      } else {
    ...
    try {
      const response = await fetch('/api/articlesPublish', {
        ...
        body: JSON.stringify({
          ...
          site_article: siteArticle[news_id], // Adicionando site_article aqui
        }),
      });
      ...
      console.log('Dados enviados para atualização:', {
        ...
        is_visible: isVisible,
        publicated_date: dateTime[news_id],
        site_article: siteArticle[news_id],
      });
  ...
  const handleSiteArticleChange = (news_id, value) => {
    setSiteArticle((prev) => ({
      ...prev,
      [news_id]: value,
    }));
                  ...
                  <Flex mt={2} m={2} p={2} direction="column">
                    ...
                    <FormControl>
                      ...
                      <select
                        ...
                        value={siteArticle[news.id] || 0}
                        onChange={(e) => handleSiteArticleChange(news.id, e.target.value)}
                      >
